...
public class UnityNotificationManager extends BroadcastReceiver
{
    	...
    	, int scheduleMode)
    {
    	try
    	{
	        ...
	        // NOTE: It seems that there's a problem with putting Serializable objects directly into Intent's extra that will be scheduled to AlarmManager.
	        // That's why we're doing this using extra step by manually serializing it to array and then putting an array to extra.
	        // Read http://stackoverflow.com/a/39209699 for details.
	        intent.putExtra("data", data.Serialize());  
    	...
    	{
    		Log.e(UnityNotificationManager.class.getSimpleName(), "Got exception while trying to schedule notification.", ex);
    	}
    }
    ...
    public static boolean CancelNotification(String id, boolean cancelPending, boolean cancelShown)
    {
    	...
    	{
    		Log.e(UnityNotificationManager.class.getSimpleName(), "Got exception while trying to cancel notification.", ex);
    		return false;
    	}
    }
    ...
    public static void CancelAllShownNotifications()
    {
		...
		{
			Log.e(UnityNotificationManager.class.getSimpleName(), "Got exception while trying to cancel all notifications.", ex);
		}
    }
    ...
    public void onReceive(Context context, Intent intent)
    {
    	try
    	{
	    	...
	    	NotificationData data = NotificationData.Deserialize(intent.getByteArrayExtra("data"));
	    	
	        Resources res = context.getResources();
	
	        Class<?> unityClassActivity = GetMainActivityClassName(context);
		...
		{
			Log.e(UnityNotificationManager.class.getSimpleName(), "Got exception while trying to show notification.", ex);
		}
    }
